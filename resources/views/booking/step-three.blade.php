...
<div class="glass-panel rounded-3xl max-w-6xl mx-auto overflow-hidden shadow-2xl">
    ...
    <div class="p-8 md:p-12 flex flex-col h-full" x-data="dateTimePicker('{{ $now }}', {{ json_encode($todayForJs) }}, {{ $extendedSession ? 'true' : 'false' }})">
            ...
            <form action="{{ route('booking.store.step-three') }}" method="POST" class="flex flex-col flex-1 text-black">
                @csrf
                <input type="hidden" name="booking_date" x-ref="bookingDate">
                <input type="hidden" name="booking_time" x-ref="bookingTime">
                ...
                <div class="mt-auto flex justify-between">
                    <a href="{{ route('booking.create.step-two') }}" class="bg-white border-2 border-emerald-400 text-emerald-600 font-bold py-3 px-8 rounded-full shadow-lg hover:bg-emerald-50 hover:text-emerald-700 transition-all transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-emerald-300">&larr; Back</a>
...
function dateTimePicker(serverTime, todayForJs, isExtended) { // Accept the new isExtended parameter
    return {
        ...
        selectDate(day) {
            ...
            this.selectedTime = '';
            this.$refs.bookingDate.value = this.selectedDate;
            this.$refs.bookingTime.value = '';
            this.fetchAvailability();
        },
        selectTime(time) {
            this.selectedTime = time;
            this.$refs.bookingDate.value = this.selectedDate;
            this.$refs.bookingTime.value = time;
        },
        fetchAvailability() {
            this.loading = true;
            this.availableSlots = [];
