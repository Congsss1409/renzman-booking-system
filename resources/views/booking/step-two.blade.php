...
<div x-data="{ showModal: false, selectedTherapist: '' }" class="glass-panel rounded-3xl max-w-6xl mx-auto overflow-hidden shadow-2xl">
    ...
    <div class="p-4 sm:p-6 md:p-12 w-full">
            ...
            <div class="mb-8">
                ...
                <div class="w-full bg-emerald-200/60 rounded-full h-2.5">
                    <div class="bg-emerald-500 h-2.5 rounded-full transition-all duration-300" style="width: 40%"></div>
                </div>
            </div>
            ...
            <form x-ref="stepTwoForm" action="{{ route('booking.store.step-two') }}" method="POST">
                ...
                <div class="space-y-4 max-h-[40vh] overflow-y-auto pr-0 md:pr-4 text-black">
                    ...
                    @forelse($therapists as $therapist)
                        <label class="block text-black w-full">
                            <input type="radio" name="therapist_id" value="{{ $therapist->id }}" @change="selectedTherapist = $event.target.value" class="hidden peer" required>
                            <div class="p-4 rounded-lg border border-white/30 cursor-pointer peer-checked:bg-white peer-checked:text-teal-600 peer-checked:ring-2 peer-checked:ring-white hover:bg-white/20 transition-all text-black flex flex-col sm:flex-row sm:items-center sm:justify-between">
                                <div class="flex items-center justify-between">
                                    ...
                                    @if($therapist->current_status == 'In Session')
                                    ...
                                    @elseif($therapist->current_status == 'Available' && $isOpen)
                                        <div class="text-left sm:text-right">
                ...
                <div class="mt-10 flex justify-between">
                    <a href="{{ route('booking.create.step-one') }}" class="bg-white/40 text-teal-700 font-bold py-3 px-10 rounded-full border border-teal-200 shadow hover:bg-white/60 transition-all transform hover:scale-105">
                        &larr; Back
                    </a>
    ...
    <div x-show="showModal" @keydown.escape.window="showModal = false" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70" style="display: none;">
        <div x-show="showModal" x-transition class="bg-teal-700 text-white rounded-2xl shadow-2xl max-w-sm mx-auto p-8" @click.away="showModal = false">
            <h3 class="text-2xl font-bold mb-2">Would you like more time?</h3>
            <p class="text-cyan-100 mb-6">Add an extra hour to your session for a longer, more relaxing experience. We'll show you available time slots for the extended session in the next step.</p>
