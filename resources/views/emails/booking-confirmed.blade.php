  ...
  <div class="wrapper">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <ul class="info-list">
          <li><span class="info-label">Service</span>{{ $b->service->name ?? 'N/A' }}</li>
          <li><span class="info-label">Therapist</span>{{ $b->therapist->name ?? 'N/A' }}</li>
          <li><span class="info-label">Date &amp; Time</span>{{ optional($b->start_time)->format('F d, Y \a\t h:i A') }}</li>
          <li><span class="info-label">Branch</span>{{ $b->branch->name ?? 'N/A' }}</li>
          <li><span class="info-label">Price</span>₱{{ number_format($b->price ?? 0, 2) }}</li>
        </ul>
      ...
      <div class="card-footer">&copy; {{ date('Y') }} Renzman. All rights reserved.</div>
    </div>
  </div>
</body>
</html>